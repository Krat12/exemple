...
import com.bank.example.model.operation.*;
...
//        this.atmWithdrawService = atmWithdrawService;
    ...
    private void addRefill() {
        for (Account account : accounts) {
            ...
            Transaction transaction = new Transaction(amount,dateTime,account);
            transaction.setTypeTransaction(TypeTransaction.REFILL);
    ...
    private void addInterests() {
        for (Map.Entry<Account, List<Deposit>> accountDepositsEntry : accountDepositMap.entrySet()) {
            ...
            for (Deposit deposit : depositList) {
                ...
                Transaction transaction = new Transaction(amount,dateTime,account);
                transaction.setTypeTransaction(TypeTransaction.INTERESTS);
                ...
                OtherProfitUtilDto dto = OtherProfitUtilDto.builder()
                        ...
                        .amount(amount)
                        .dateTime(interests.getTransaction().getDateTime())
                        .accountId(account.getId())
                        .build();
        ...
        for (Map.Entry<CashBackCompany, List<Account>> cashBackCompanyAccountEntry : cashBackCompanyAccountMap.entrySet()) {
            ...
            for (Account account : accountList) {
                ...
                for (int i = 0; i < cashBackAmount; i++) {
                    ...
                    Transaction transaction = new Transaction(amount,dateTime,account);
                    transaction.setTypeTransaction(TypeTransaction.CASH_BACK);
                    cashBack.setTransaction(transaction);
                    cashBack.setCashBackCompany(cashBackCompany);
                    cashBackService.persist(cashBack);

                    OtherProfitUtilDto dto = OtherProfitUtilDto.builder()
                            .id(cashBack.getId())
                            .otherProfitType(TransactionUtilType.CASH_BACK)
                            .amount(cashBack.getTransaction().getAmount())
                            .dateTime(dateTime)
                            .accountId(account.getId())
                            .build();
